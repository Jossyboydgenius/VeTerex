    ...
    <div className="px-4 py-6 space-y-6">
      ...
      >
        ...
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-dark-800 text-sm">
          ...
          <span className="text-dark-300">{currentAccount?.network || 'Ethereum'}</span>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="grid grid-cols-5 gap-2">
          {(['book', 'movie', 'anime', 'manga', 'tvshow'] as const).map((type) => {
            ...
            return (
              <div key={type} className="text-center">
                ...
                `}>
                  <Icon className={`w-5 h-5 ${count > 0 ? 'text-accent-400' : 'text-dark-500'}`} />
                </div>
        ...
        {wepinUser?.userInfo?.provider && (
          <div className="card flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-white capitalize">{wepinUser.userInfo.provider}</p>
            </div>
          </div>
        )}
