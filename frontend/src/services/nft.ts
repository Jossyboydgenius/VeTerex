...
export function mapMediaKind(kind: number): 'book' | 'movie' | 'anime' | 'comic' | 'manga' | 'tvshow' | 'unknown' {
...
export async function getSimilars(user: HexAddress, tokenIds: bigint[]): Promise<HexAddress[]> {
...
export async function mintCompletion(to: HexAddress, kind: number, uri: string, name: string) {
  const pk = (import.meta.env.VITE_BACKEND_PRIVATE_KEY || import.meta.env.BACKEND_PRIVATE_KEY || '').trim()
...
export async function getGroupMemberAt(mediaId: Hex, index: bigint): Promise<HexAddress> {
...
export async function mediaInfo(mediaId: Hex): Promise<[boolean, number, string]> {
  const info = await readContract(wagmiConfig, {
    address: VETEREX_CONTRACT.address as HexAddress,
    abi: VETEREX_ABI,
    functionName: 'mediaInfo',
